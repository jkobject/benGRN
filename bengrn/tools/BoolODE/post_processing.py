...
def genSamples(opts):
    ...
    for did in range(1, opts["nDatasets"] + 1):
        ...
        # Create cell ids
        simids = np.random.choice(
            range(num_simulations), size=sample_size, replace=False
        )
...
def doDimRed(opts):
    ...
    DimRedDF = pd.DataFrame(
        DimRedRes, columns=["dim1", "dim2"], index=pd.Index(list(ExpDF.columns))
    )
    DimRedDF.loc[:, "pt"] = ptDF.min(axis="columns")
    DimRedDF.to_csv(
        str(opts["expr"].parent) + "/tsne" + str(perplexity) + ".tsv", sep="\t"
    )
    plt.figure()
    ...
    *Author: Aditya Pratapa*
    ...
    if nClust == 1:
        ...
        endClust = ",".join(
            [
                str(ix)
                for ix in DimRedDF.groupby("cl").mean()["pt"].index
                if ix != startClust
            ]
        )
        ...
        if noEnd:
            cmdToRun = " ".join(
                [
                    "docker run --rm -v",
                    str(Path.cwd()) + "/" + outPath + "/:/data/temp",
        ...
        tn = pd.read_csv(outPath + "/rd.tsv", header=0, index_col=None, sep="\t")

        cl = pd.read_csv(outPath + "/cl.tsv", header=0, index_col=0, sep="\t")
        ...
        detPT = pd.read_csv(outPath + "/SlingshotPT.csv", header=0, index_col=0)
        print()
        ...
        for line in range(0, lneCnt, 2):
            sns.lineplot(
                x=curveLst[line + 1], y=curveLst[line], color="k", ax=axes[1][0]
            )

        sns.scatterplot(
            x="dim1", y="dim2", data=tn, hue="kMeans", palette="Set1", ax=axes[1][1]
        )

        # Plot deterministic pseduotime
        # and original clusters
        detPT = pd.read_csv(outPath + "/PseudoTime.csv", header=0, index_col=0)
        colNames = detPT.columns
        ...
        sns.scatterplot(
            x="dim1", y="dim2", data=tn, hue="ExpTime", palette="viridis", ax=axes[0][0]
        )

        sns.scatterplot(
            x="dim1", y="dim2", data=tn, hue="Original", palette="Set1", ax=axes[0][1]
        )
        ...
        tn.to_csv(outPath + "/Updated_rd.tsv", sep="\t")
