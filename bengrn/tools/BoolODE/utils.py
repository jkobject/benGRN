...
def loadParameterValues():
    ...
    """
    with open(
        str(Path(__file__).parent.absolute()) + "/parameters.yaml", "r"
    ) as parameterfile:
    ...
    boolodeDefaults = {
        "mRNATranscription": 20.0,
        "mRNADegradation": 10.0,
        "proteinTranslation": 10.0,
        "proteinDegradation": 1.0,
        "heavisideSigma": 10.0,
        "signalingTimescale": 5.0,
        "hillCoefficient": 10.0,
    ...
    # Max level checks
    x_max = (
        kineticParameterDefaults["mRNATranscription"]
        / kineticParameterDefaults["mRNADegradation"]
    )
    y_max = x_max * (
        kineticParameterDefaults["proteinTranslation"]
        / kineticParameterDefaults["proteinDegradation"]
    )
...
def getSaneNval(size, lo=1.0, hi=10.0, mu=2.0, sig=2.0, identicalPars=False):
    """
...
def generateInputFiles(
    resultDF, BoolDF, withoutRules, parameterInputsDF, tmax, numcells, outPrefix=""
):
    """
    ...
    for g in genes:
        ...
        regulators = [
            t for t in tokens if (t in genes or t in inputs) if t not in avoidthese
        ]
        if "not" in tokens:
    ...
    if len(resultDF.columns) < 1e3:
        ...
        expdf = pd.DataFrame(
            columns=["E" + str(i) + "_" + str(times[i]) for i in range(numcells)],
...
def sampleTimeSeries(
    ...
    if writeProtein:
        ...
        for ri in varmapper.keys():
            sampleDict[varmapper[ri]] = {
                h: P[ri][ti] for h, ti in zip(experimentTimePoints, timeIndex)
            }
    else:
        # Default, only mRNA
        if len(proteinlist) == 0:
            ...
            for si in speciesoi:
                sampleDict[varmapper[si]] = {
                    h: P[si][ti] for h, ti in zip(experimentTimePoints, timeIndex)
                }
