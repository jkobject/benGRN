...
class GenerateModel:
    ...
    def __init__(
        self, settings, parameterInputsDF, parameterSetDF, interactionStrengthDF
    ) -> None:
        """
    ...
    def readBooleanRules(self):
        ...
        for n in self.withoutRules:
            if (
                not self.parameterInputsDF.empty
            ...
            self.df = self.df.append(
                {
                    "Gene": "dummy" + str(dg),
                    "Rule": " or ".join(
                        [s for s in np.random.choice(list(allnodes), size=max_parents)]
                    ),
                },
                ignore_index=True,
            )
    ...
    def getParameters(self, signalingtimescale):
        ...
        x_max = (
            self.kineticParameterDefaults["mRNATranscription"]
            / self.kineticParameterDefaults["mRNADegradation"]
        )
        y_max = x_max * (
            self.kineticParameterDefaults["proteinTranslation"]
            / self.kineticParameterDefaults["proteinDegradation"]
        )

        hillThreshold = y_max / 2
        heavisideOmega = 2.0 / y_max
        ...
        if self.settings["sample_pars"]:
            self.assignSampledParameterValues(
                parameterNamePrefixAndDefaultsAll, parameterNamePrefixAndDefaultsGenes
            )
        else:
            self.assignDefaultParameterValues(
                parameterNamePrefixAndDefaultsAll, parameterNamePrefixAndDefaultsGenes
            )
    ...
    def addParameterInputs(self):
        ...
        if self.settings["modeltype"] == "hill":
            self.par.update(
                {"k_" + p: hillThreshold for p in parameterInputsDict.keys()}
            )
            self.par.update(
                {"n_" + p: hillCoefficient for p in parameterInputsDict.keys()}
            )
                ...
                self.par.update(
                    {
                        ...
                        + regulator
                        + "_"
                        + target: hillThreshold / interactionStrength
                    }
                )
    ...
    def assignDefaultParameterValues(
        self, parameterNamePrefixAndDefaultsAll, parameterNamePrefixAndDefaultsGenes
    ):
        """
    ...
    def assignSampledParameterValues(
        self, parameterNamePrefixAndDefaultsAll, parameterNamePrefixAndDefaultsGenes
    ):
        """
        ...
        for parPrefix, parDefault in parameterNamePrefixAndDefaultsAll.items():
            sampledParameterValues = utils.getSaneNval(
                len(self.withRules),
    ...
    def createRegulatoryTerms(self, currgene, combinationOfRegulators, regSpecies):
        ...
        if not self.interactionStrengthDF.empty:
            if currgene in self.interactionStrengthDF["Gene1"].values:
                ...
                regulatorsWithStrength = set(
                    self.interactionStrengthDF[
                        self.interactionStrengthDF["Gene1"] == currgene
                    ]["Gene2"].values
                )
        if self.settings["modeltype"] == "hill":
    ...
    def generateModelDict(self):
        ...
        for i, row in self.df.iterrows():
            ...
            if self.settings["modeltype"] == "hill":
                ...
                self.par["omega_" + row["Gene"]] = utils.heavisideThreshold(
                    boolodespace["booleval"]
                )
        # End initialization
        ...
        for i, row in self.df.iterrows():
            ...
            allreg, regSpecies, regInputs = utils.getRegulatorsInRule(
                row["Rule"], self.withRules, self.inputs
            )
            ...
            for i in range(1, len(allreg) + 1):
                for combinationOfRegulators in combinations(allreg, i):
                    regulatorExpression = self.createRegulatoryTerms(
                        currgene, combinationOfRegulators, regSpecies
                    )
                    if self.settings["modeltype"] == "hill":
                        ...
                        num += (
                            ...
                            + currgene
                            + "_"
                            + "_".join(list(combinationOfRegulators))
                            + "*"
                            + regulatorExpression
                        )
                    elif self.settings["modeltype"] == "heaviside":
                    ...
                    if self.settings["modeltype"] == "hill":
                        ...
                        self.par[
                            ...
                            + "_"
                            + "_".join(list(combinationOfRegulators))
                        ] = self.kineticParameterDefaults[
                            "heavisideOmega"
                        ] * utils.heavisideThreshold(
                            boolodespace["boolval"]
                        )
            ...
            if currgene in self.proteinlist:
                ...
                self.varspecs["p_" + currgene] = (
                    "signalingtimescale*(y_max*" + Production + "-" + Degradation + ")"
                )
            else:
                Production = "m_" + currgene + "*" + f
                Degradation = "l_x_" + currgene + "*x_" + currgene
                self.varspecs["x_" + currgene] = Production + "-" + Degradation
        ...
        xvals = [1.0 for _ in range(len(self.genelist))]
        pvals = [20.0 for _ in range(len(self.proteinlist))]
        ics = {}
        ...
        self.varmapper = {
            i: var for i, var in enumerate(self.ModelSpec["varspecs"].keys())
        }
