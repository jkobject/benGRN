...
def Experiment(
    mg, Model, tspan, settings, icsDF, writeProtein=False, normalizeTrajectory=False
):
    """
    ...
    if settings["sample_cells"]:
        ...
        header = [
            "E" + str(cellid) + "_" + str(time)
            for cellid, time in zip(range(settings["num_cells"]), sampleAt)
        ]
    ...
    for cellid in tqdm(range(settings["num_cells"])):
        if settings["sample_cells"]:
            df = pd.read_csv(
                outPrefix + "/simulations/E" + str(cellid) + "-cell.csv", index_col=0
            )
            df = df.sort_index()
        else:
            df = pd.read_csv(
                outPrefix + "/simulations/E" + str(cellid) + ".csv", index_col=0
            )
            df = df.sort_index()
    ...
    if settings["nClusters"] > 1:
        ...
        clusterLabels = (
            KMeans(n_clusters=settings["nClusters"], n_jobs=8)
            .fit(groupedDF.T.values)
            .labels_
        )
        print("Clustering took %0.3fs" % (time.time() - start))
        clusterDF = pd.DataFrame(
            data=clusterLabels, index=groupedDF.columns, columns=["cl"]
        )
...
def simulateAndSample(argdict):
    ...
    while retry:
        seed += 1000
        y0_exp = simulator.getInitialCondition(
            ss,
            ModelSpec,
            rnaIndex,
            proteinIndex,
        ...
        df = pd.DataFrame(
            ...
            columns=["E" + str(cellid) + "_" + str(i) for i in tps],
        )
        ...
        if sampleCells:
            ...
            sampledf = utils.sampleCellFromTraj(
                cellid,
                tspan,
                P,
                varmapper,
